...
<template>
    <AppLayout title="List Kuisioner">
        <template #header>
            <HeaderText :href="route('projects', clientSlug)">
                        ...
                        <thead class="text-xs text-center text-white uppercase bg-primary">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3"
                                    v-if="$page.props.auth.user.current_team_id != 1 || props.user.usertype == 'superadmin'">
                                    Target</th>
                            ...
                            <tr v-for="survey in filteredSurveys" :key="survey.id"
                                    ...
                                    <ul>
                                        ...
                                        <li v-for="(province, index) in getCitiesAndRegencies(survey)" :key="index">
                                            <div class="flex justify-between items-center mt-1 mb-1">
                                                <span>- {{ province.province_name }} <b>({{ province.target_response
                                                        }})</b></span>
                                                <svg v-if="province.hasCitiesOrRegencies"
                                        ...
                                        <div v-if="$page.props.auth.user.usertype === 'admin' || $page.props.auth.user.usertype === 'superadmin'"
                                            ...
                                            <a :href="route('location_surveys', [clientSlug, projectSlug, survey.id])"
                                                class="font-medium text-blue-600 dark:text-blue-500 hover:underline"><br>
                                                Details click here
                                            </a>
                                        </div>
                                    </ul>
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-xl"
                                        ...
                                        <div class="square">
                                            {{ getSurveySubmissions(survey.id) }}/<b> {{
                                                getSelectedProvinces(survey).total
                                            }}</b><br>
                                            <br>
                                        </div>
                                    </div>
                                ...
                                <td class="px-6 py-6">
                                    <div class="grid grid-cols-2 gap-x-2 justify-center content-center">
                                        ...
                                        <NavLinkBlue
                                            :class="props.user.current_team_id == 1 || props.user.current_team_id == 2 ? '' : 'col-span-2'"
                                            :href="route('response', [clientSlug, projectSlug, survey.id])"
                                            v-if="$page.props.auth.user.current_team_id != 1 || props.user.usertype == 'superadmin'"
                                            class="w-full flex justify-center py-2.5 text-white bg-secondary rounded-md text-sm hover:bg-transparent hover:text-secondary focus:hover:!ring-secondary hover:!ring-secondary focus:!ring-secondary transition ">
                                            Cek Respon
                                        </NavLinkBlue>
