    ...
    <AppLayout title="List Projects">
        ...
        <main class="min-h-screen bg-repeat bg-[('/img/bg-dashboard.png')]">
                ...
                <div class="flex justify-between items-center mb-5">
                    <div class="w-1/2 sm:w-full">
                        <NavLink :href="route('create_projects')" 
                            class="bg-primary text-white font-medium text-sm px-6 py-2 rounded-md border-2 hover:bg-white hover:text-primary hover:border-primary transition">Add Projects
                        </NavLink>
                    </div>
            ...
            <div class="container mx-auto grid sm:grid-cols-2 lg:grid-cols-3 gap-10">
                <div v-for="project in projects" class="relative rounded-md shadow-lg outline outline-2 outline-gray-300 px-5 py-4 h-80 bg-white mx-5 sm:mx-0">
                    ...
                    <div class="absolute bottom-0 right-0 left-0 p-5 mx-5 sm:mx-0">
                        ...
                        <div v-if="$page.props.auth.user.usertype === 'admin' || $page.props.auth.user.usertype === 'superadmin'" class="relative inline-block text-left w-full">
                            <div class="flex justify-end">
                                <Dropdown>
                                    <template #trigger>
                                        <div class="inline-flex w-12 gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 cursor-pointer">
                                            <div class="flex">
                                    ...
                                    <template #content>
                                            <div class="py-1">
                                                <a :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-sm']" :href="route('edit_projects',project.id)">Edit Project</a>
    
                                                <a :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-sm']" @click="hapus(project.id)" class="cursor-pointer">Hapus Project</a>
                                            </div>
                                    </template>
                                </Dropdown>
                            </div>
