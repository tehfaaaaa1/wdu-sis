...
<template>
    <AppLayout title="Add Contact">
        <template #header>
            <HeaderText :href="route('recipient-details', recipient.slug)">
                <span class="text-black">Recipient - </span> {{ recipient.name }}
            </HeaderText>
        </template>
        <main class="min-h-screen scroll-smooth">
            <div
                ...
                <form @submit.prevent="importContact" class="p-3">
                    ...
                    <div class="flex items-center gap-x-5 mb-1">
                    ...
                    <a href="/excel/template.xlsx" download class="text-blue-500 underline">Download Excel Template</a>
                </form>
                <div class="block">
                    <form @submit.prevent="addcontact">
                        <div class="relative overflow-x-auto sm:rounded-sm gap-x-3">
                            <table
                                class="w-full text-sm text-left rtl:text-right border-spacing-[-1px] text-gray-500 border-separate p-3">
                                <thead class="text uppercase">
                                    <tr class="border border-gray-300">
                                        <th scope="col"
                                            class="sticky left-0 border border-gray-300 px-6 py-3 z-10 bg-white">Email
                                        </th>
                                        ...
                                        <th scope="col" class="px-6 py-3 border-y border-r border-gray-300">Occupation
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr class="" v-for="(contact, index) in contacts" :key="index">
                                        <td
                                            class="bg-white sticky left-0 px-4 py-3 font-medium text-gray-900 border border-t-0 border-gray-300 shadow-right">
                                            <input class="placeholder:text placeholder:font-normal" type="email"
                                                placeholder="Email" v-model="contacts[index].email" />
                                        </td>
                                        <td class="px-4 py-3 border-b border-gray-200 font-medium text-gray-900">
                                            <input class="placeholder:text-sm placeholder:font-normal" type="text"
                                                placeholder="First Name" v-model="contacts[index].first_name" />
                                        </td>
                                        <td class="px-4 py-3 border-b border-gray-200 font-medium text-gray-900">
                                            <input class="placeholder:text-sm placeholder:font-normal" type="text"
                                                placeholder="Last Name" v-model="contacts[index].last_name" />
                                        </td>
                                        <td class="px-4 py-3 border-b border-gray-200 font-medium text-gray-900">
                                            <input class="placeholder:text-sm placeholder:font-normal" type="text"
                                                placeholder="Company" v-model="contacts[index].company" />
                                        </td>
                                        <td
                                            class="px-4 py-3 border-b border-r border-gray-200 font-medium text-gray-900">
                                            <input class="placeholder:text-sm placeholder:font-normal" type="text"
                                                placeholder="Occupation" v-model="contacts[index].occupation" />
                                        </td>
                                    </tr>
                                    <tr class="border-2 border-gray-200">
                                        <td class="bg-white sticky left-0 py-4 font-medium text-gray-900">
